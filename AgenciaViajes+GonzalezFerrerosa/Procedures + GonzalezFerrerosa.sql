...
DROP PROCEDURE IF EXISTS yeet_or_add_client;
DROP PROCEDURE IF EXISTS yeet_or_add_seller;
DELIMITER //
...
CREATE PROCEDURE yeet_or_add_client (IN DELETEID_OR_INSERT_0 INT,IN client_name VARCHAR(30),IN client_surname VARCHAR(30),IN client_id int
...
CREATE PROCEDURE yeet_or_add_seller(IN DELETEID_OR_INSERT_0 INT,IN seller_id int,IN seller_name VARCHAR(30),IN seller_surname VARCHAR(30))
BEGIN
	DECLARE D_OR_I INT;
	SET D_OR_I =DELETEID_OR_INSERT_0;
...
call travel_lines.yeet_or_add_client(0,'Testing','MCtest','1036687074','','');# id si se quiere borrar,Nombre,apellido,cedula ID, email, telefono

call travel_lines.yeet_or_add_seller(0,'1234567810','dummy','jonhdoe');##si borra el id es diferente a 0, el nombre, apellido y cdeula id